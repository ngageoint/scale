...
class TestProcessJobInput(TransactionTestCase):
    ...
    def test_execute_with_recipe_and_optional_data(self):
        """Tests calling ProcessJobInput.execute() successfully when a job has to get its data from its recipe"""
        ...
        manifest_1 = {
            ...
            'job': {
                ...
                'interface': {
                    ...
                    'inputs': {'files': [], 'json': []},
                    'outputs': {
                        'files': [{'name': 'OUTPUT_A', 'pattern': '*.png', 'required': False}]
                    }
                }
            }
        }
        ...
        manifest_2 = {
            ...
            'job': {
                ...
                'interface': {
                    'command': '',
                    'inputs': {'files': [{'name': 'INPUT_B', 'multiple': False, 'required' : False}]},
                    'outputs': {
                        'files': [{'name': 'OUTPUT_B', 'pattern': '*.png'}]
                    }
                }
        ...
        job_1 = job_test_utils.create_job(job_type=job_type_1, num_exes=1, status='COMPLETED')
        job_2 = job_test_utils.create_job(job_type=job_type_2, num_exes=0, status='PENDING', input_file_size=None,
                                          input=None)
        ...
        # Check job for expected input_file_size
        self.assertEqual(job_2.input_file_size, 0.0)
        # Check job for no input data
        self.assertEqual(len(job_2.get_input_data().values), 0)
