...
class TestPendingJobs(TransactionTestCase):
    ...
    def test_json(self):
        ...
        new_message = PendingJobs.from_json(message_json_dict)
        result = new_message.execute()

        self.assertTrue(result)
        jobs = Job.objects.filter(id__in=job_ids).order_by('id')
        self.assertEqual(jobs[0].status, 'PENDING')
        self.assertEqual(jobs[0].last_status_change, original_status_change)
        self.assertEqual(jobs[1].status, 'PENDING')
        self.assertEqual(jobs[1].last_status_change, status_change)
    ...
    def test_execute(self):
        ...
        self.assertTrue(result)
        jobs = Job.objects.filter(id__in=job_ids).order_by('id')
        # Job 1 should not have been updated since it was already PENDING
        self.assertEqual(jobs[0].status, 'PENDING')
        self.assertEqual(jobs[0].last_status_change, original_status_change)
        # Job 2 should have been successfully updated to PENDING
        self.assertEqual(jobs[1].status, 'PENDING')
        self.assertEqual(jobs[1].last_status_change, status_change)
        # Job 3 should have been successfully updated to PENDING
        self.assertEqual(jobs[2].status, 'PENDING')
        self.assertEqual(jobs[2].last_status_change, status_change)
