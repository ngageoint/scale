...
class CleanupTask(NodeTask):
    ...
    def __init__(self, framework_id, agent_id, job_exes):
        ...
        all_scale_dangling_volumes_cmd = 'docker volume ls -f dangling=true -f name=scale_ -q'
        ...
        if self._is_initial_cleanup:
            ...
            # Container/volume lists are generated by filtering list from Docker by the specific
            # containers/volumes we are looking for
            if containers:
                #container_list_cmd = all_containers_cmd
                container_list_cmd = '%s -f name=%s' % (all_containers_cmd, ' -f name='.join(containers))
                #for c in containers:
                    #container_list_cmd += ' -f name=%s' % c
            else:
                container_list_cmd = ':'
            if volumes:
