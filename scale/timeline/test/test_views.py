...
class TestRecipeTypeTimelineView(APITestCase):
    ...
    def test_type_ids(self):
        ...
        url = '/%s/timeline/recipe-types/?started=%s&ended=%s&recipe_type_name=%s' % (self.api, started, ended,
    ...
    def test_type_names(self):
        ...
        url = '/%s/timeline/recipe-types/?started=%s&ended=%s&recipe_type_id=%s' % (self.api, started, ended,
...
class TestJobTypeTimelineView(APITestCase):
    ...
    def setUp(self):
        ...
        manifest['job']['interface']['inputs'] = {
            'files': [{'name': 'INPUT_FILE', 'required': True, 'mediaTypes': ['image/png'], 'partial': False}]}
        self.job_type_1 = job_test_utils.create_seed_job_type(manifest=manifest)
