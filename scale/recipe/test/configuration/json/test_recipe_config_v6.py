...
class TestRecipeConfigurationV6(TestCase):
    ...
    def test_convert_config_to_v6_json(self):
        ...
        RecipeConfigurationV6(config=json.get_dict(), do_validate=True)  # Revalidate
    ...
    def test_init_validation(self):
        ...
        jc = RecipeConfigurationV6(config=config_dict, existing=existing, do_validate=True).get_configuration()
        vol1 = jc.get_mount_volume('mount_1', 'test1', '/test/path', 'ro')
        self.assertEqual(vol1.name, 'test1')
        self.assertEqual(vol1.container_path, '/test/path')
        self.assertEqual(vol1.mode, 'ro')
