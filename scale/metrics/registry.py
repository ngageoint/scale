...
class MetricsType(object):
    ...
    def _get_column_map(self):
        ...
        :rtype: dict[str, :class:`metrics.registry.MetricsTypeColumn`]
        '''
...
class MetricsTypeFilter(object):
    ...
    :type param: str
    :keyword type: The data type of the filter parameter. Examples: bool, date, datetime, float, int, string, time
    :type type: str
    '''
...
class MetricsPlotValue(object):
    ...
    :keyword id: The unique identifier of the choice model associated with the value.
    :type id: str
...
def register_provider(provider, serializer_class=None):
    ...
    if metrics_type.name in _PROVIDERS:
        logger.warn(u'Duplicate metrics type definition registered for name: %s', metrics_type.name)
    logger.debug(u'Registering metrics type: %s -> %s', metrics_type.name, metrics_type)
