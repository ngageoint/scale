...
class TestUpdateBatchMetrics(TestCase):
    ...
    def test_json(self):
        ...
        a = recipe_test_utils.create_recipe_node(recipe=recipe_1)
        b = recipe_test_utils.create_recipe_node(recipe=recipe_1)
        c = recipe_test_utils.create_recipe_node(recipe=recipe_1)
        d = recipe_test_utils.create_recipe_node(recipe=recipe_1)
        e = recipe_test_utils.create_recipe_node(recipe=recipe_1)
        RecipeNode.objects.bulk_create([a,b,c,d,e])
        job_1 = job_test_utils.create_job(status='FAILED', recipe=recipe_1, recipe_node=a)
        job_2 = job_test_utils.create_job(status='CANCELED', recipe=recipe_1, recipe_node=b)
        job_3 = job_test_utils.create_job(status='BLOCKED', recipe=recipe_1, recipe_node=c)
        job_4 = job_test_utils.create_job(status='BLOCKED', recipe=recipe_1, recipe_node=d)
        job_5 = job_test_utils.create_job(status='COMPLETED', recipe=recipe_1, recipe_node=e)


        recipe_2 = recipe_test_utils.create_recipe(batch=batch)
        recipe_2.is_completed = True
        recipe_2.save()
        a = recipe_test_utils.create_recipe_job(recipe=recipe_2)
        b = recipe_test_utils.create_recipe_job(recipe=recipe_2)
        c = recipe_test_utils.create_recipe_job(recipe=recipe_2)
        RecipeNode.objects.bulk_create([a, b, c])
        job_6 = job_test_utils.create_job(status='COMPLETED', recipe=recipe_2, recipe_node=a)
        job_7 = job_test_utils.create_job(status='COMPLETED', recipe=recipe_2, recipe_node=b)
        job_8 = job_test_utils.create_job(status='COMPLETED', recipe=recipe_2, recipe_node=c)
    ...
    def test_execute(self):
        ...
        a = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='a')
        b = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='b')
        c1 = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='c')
        c2 = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='c')
        c3 = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='c')
        d1 = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='d')
        d2 = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='d')
        e1 = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='e')
        e2 = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='e')
        f = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='f')
        g = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='g')
        h = recipe_test_utils.create_recipe_node(recipe=recipe_1, node_name='h')
        RecipeNode.objects.bulk_create([a,b,c1,c2,c3,d1,d2,e1,e2,f,g,h])

        job_1 = job_test_utils.create_job(status='COMPLETED', started=started, ended=ended_1, recipe=recipe_1, recipe_node=a)
        job_2 = job_test_utils.create_job(status='COMPLETED', recipe=recipe_1, recipe_node=b)
        ...
        job_3 = job_test_utils.create_job(status='QUEUED', recipe=recipe_1, recipe_node=c1)
        job_4 = job_test_utils.create_job(status='QUEUED', recipe=recipe_1, recipe_node=c2)
        job_5 = job_test_utils.create_job(status='RUNNING', recipe=recipe_1, recipe_node=c3)
        job_6 = job_test_utils.create_job(status='RUNNING', recipe=recipe_1, recipe_node=d1)
        job_7 = job_test_utils.create_job(status='RUNNING', recipe=recipe_1, recipe_node=d2)
        job_8 = job_test_utils.create_job(status='PENDING', recipe=recipe_1, recipe_node=e1)
        job_9 = job_test_utils.create_job(status='PENDING', recipe=recipe_1, recipe_node=e2)
        job_10 = job_test_utils.create_job(status='CANCELED', recipe=recipe_1, recipe_node=f)
        job_11 = job_test_utils.create_job(status='BLOCKED', recipe=recipe_1, recipe_node=g)
        job_12 = job_test_utils.create_job(status='FAILED', recipe=recipe_1, recipe_node=h)

        recipe_2 = recipe_test_utils.create_recipe(batch=batch, recipe_type=recipe_type)
        recipe_2.is_completed = True
        recipe_2.save()
        a = recipe_test_utils.create_recipe_node(recipe=recipe_2, node_name='a')
        b = recipe_test_utils.create_recipe_node(recipe=recipe_2, node_name='b')
        c1 = recipe_test_utils.create_recipe_node(recipe=recipe_2, node_name='c')
        c2 = recipe_test_utils.create_recipe_node(recipe=recipe_2, node_name='c')
        d1 = recipe_test_utils.create_recipe_node(recipe=recipe_2, node_name='d')
        d2 = recipe_test_utils.create_recipe_node(recipe=recipe_2, node_name='d')
        d3 = recipe_test_utils.create_recipe_node(recipe=recipe_2, node_name='d')
        d4 = recipe_test_utils.create_recipe_node(recipe=recipe_2, node_name='d')
        e = recipe_test_utils.create_recipe_node(recipe=recipe_2, node_name='e')
        RecipeNode.objects.bulk_create([a, b, c1, c2, d1, d2, d3, d4, e])

        job_13 = job_test_utils.create_job(status='FAILED', recipe=recipe_2, recipe_node=a)
        job_14 = job_test_utils.create_job(status='COMPLETED', recipe=recipe_2, recipe_node=b)
        job_15 = job_test_utils.create_job(status='RUNNING', recipe=recipe_2, recipe_node=c1)
        job_16 = job_test_utils.create_job(status='RUNNING', recipe=recipe_2, recipe_node=c2)
        job_17 = job_test_utils.create_job(status='QUEUED', recipe=recipe_2, recipe_node=d1)
        job_18 = job_test_utils.create_job(status='QUEUED', recipe=recipe_2, recipe_node=d2)
        job_19 = job_test_utils.create_job(status='QUEUED', recipe=recipe_2, recipe_node=d3)
        job_20 = job_test_utils.create_job(status='QUEUED', recipe=recipe_2, recipe_node=d4)
        job_21 = job_test_utils.create_job(status='PENDING', recipe=recipe_2, recipe_node=e)

        recipe_3 = recipe_test_utils.create_recipe(batch=batch, recipe_type=recipe_type)
        recipe_3.is_completed = True
        recipe_3.save()
        a = recipe_test_utils.create_recipe_node(recipe=recipe_3, node_name='a')
        b = recipe_test_utils.create_recipe_node(recipe=recipe_3, node_name='b')
        c1 = recipe_test_utils.create_recipe_node(recipe=recipe_3, node_name='c')
        c2 = recipe_test_utils.create_recipe_node(recipe=recipe_3, node_name='c')
        c3 = recipe_test_utils.create_recipe_node(recipe=recipe_3, node_name='c')
        c4 = recipe_test_utils.create_recipe_node(recipe=recipe_3, node_name='c')
        RecipeNode.objects.bulk_create([a, b, c1, c2, c3, c4])

        job_22 = job_test_utils.create_job(status='COMPLETED', recipe=recipe_3, recipe_node=a)
        job_23 = job_test_utils.create_job(status='COMPLETED', recipe=recipe_3, recipe_node=b)
        task_results = {'version': '1.0', 'tasks': [{'task_id': '1234', 'type': 'main',
                                                     'started': datetime_to_string(started),
                                                     'ended': datetime_to_string(ended_3)}]}
        task_results = TaskResults(task_results=task_results, do_validate=False)
        job_test_utils.create_job_exe(job=job_23, status='COMPLETED', task_results=task_results)
        job_24 = job_test_utils.create_job(status='COMPLETED', started=started, ended=ended_2, recipe=recipe_3, recipe_node=c1)
        job_25 = job_test_utils.create_job(status='COMPLETED', started=started, ended=ended_3, recipe=recipe_3, recipe_node=c2)
        job_26 = job_test_utils.create_job(status='COMPLETED', started=started, ended=ended_4, recipe=recipe_3, recipe_node=c3)
        job_27 = job_test_utils.create_job(status='COMPLETED', recipe=recipe_3, recipe_node=c4)
