...
class DCOSServiceAuth(AuthBase):
    ...
    def token(self):
        ...
        if time.time() > self._expiration or self._token is None:
            self._acs_login()

        return self._token

    def _generate_token(self, uid, private_key, scheme='RS256', expiry_seconds=60*60):
        ...
        :return: login token
        """
        expire_time = time.time() + float(timedelta(seconds=expiry_seconds).seconds)
