...
COPY dockerfiles/framework/scale/marathon-0.10.0.tar.gz /tmp/
RUN if [ $EPEL_INSTALL -eq 1 ]; then yum install -y epel-release; fi\
 ...
 && yum install -y \
         ...
         wget \
         python-devel \
 && cd /tmp && tar xvf marathon-0.10.0.tar.gz && cd marathon-0.10.0 && python setup.py install && cd ../ && rm -fr marathon-* \
