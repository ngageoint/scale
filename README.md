...
| ------------------------ | --------------------------- | -----------------------------------------|
| SCALE_SECRET_KEY         | 'THIS IS AN INSECURE KEY'   | A key to keep your instance secure.      |
| SCALE_DEBUG              | ''                          | Change to '1' for debugging              |
| SCALE_URL_PREFIX         | ''                          | URL prefix for the REST API              |
| SCALE_ALLOWED_HOSTS      | ''                          | , separated list of hosts for django     |
| SCALE_POSTGIS_TEMPLATE   | 'template1'                 | DB template with postgis extension       |
| SCALE_DB_NAME            | 'scale'                     | database name for scale                  |
| SCALE_DB_USER_NAME       | 'postgres'                  | database login name                      |
| SCALE_DB_USER_PASSWORD   | 'postgres'                  | database login password                  |
| SCALE_DB_HOST            | use link to `db`            | database host name                       |
|                          | or 'localhost'              |                                          |
| SCALE_DB_PORT            | use link to `db`            | database port                            |
|                          | or '5432'                   |                                          |
| SCALE_NODE_WORK_DIR      | '/tmp/scale/work'           | Node work directory for slaves           |
| SCALE_MESOS_MASTER       | 'zk://localhost:2181/scale' | Mesos master location                    |
| SCALE_METRICS_DIR        | ''                          | Location for algorithm metrics.          |
| SCALE_INFLUXDB_BASE_URL  | None                        | URL for InfluxDB metrics server          |
